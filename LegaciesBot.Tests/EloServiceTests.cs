...
public class EloServiceTests
{
    ...
    public void ApplyTeamResult_WinningTeamGainsLosingTeamLoses()
    {
        ...
        var result = EloService.ApplyTeamResult(teamA, teamB, MatchResult.TeamAWin, stats, seasons);
    ...
    public void ApplyTeamResult_Draw_DoesNotGiveFullPointsToEitherTeam()
    {
        ...
        var result = EloService.ApplyTeamResult(teamA, teamB, MatchResult.Draw, stats, seasons);

        Assert.True(Math.Abs(result[1]) < 32);
        Assert.True(Math.Abs(result[3]) < 32);

        Assert.Equal(1, stats.GetOrCreate(1).Draws);
        Assert.Equal(1, stats.GetOrCreate(3).Draws);
        Assert.Equal(0, stats.GetOrCreate(1).Wins);
        Assert.Equal(0, stats.GetOrCreate(1).Losses);
    }
